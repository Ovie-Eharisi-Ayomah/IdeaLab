// Main dashboard component
export { default as IdeaLabDashboard } from './IdeaLabDashboard';

// Dashboard sections
export { default as SummarySection } from './sections/SummarySection';
export { default as SegmentationSection } from './sections/SegmentationSection';
export { default as ProblemSection } from './sections/ProblemSection';
export { default as CompetitionSection } from './sections/CompetitionSection';
export { default as MarketSection } from './sections/MarketSection';

// Shared components
export { default as ScoreCircle } from './shared/ScoreCircle';
export { default as SectionCard } from './shared/SectionCard';
export { default as InsightItem } from './shared/InsightItem';
export { default as RecommendationItem } from './shared/RecommendationItem'; 